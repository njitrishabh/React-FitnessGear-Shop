describe('Favorites Page', () => {
    ...
    it('Show No items found when favorite is empty', () => {

        cy.intercept('/components/ProductContext', { favorites: [] }).as('getFavorites')

        cy.visit('http://localhost:3000/favorites')

        cy.contains('h3', 'No Item Found').should('be.visible')
    })

    it('Show favorite products when favorites are present', () => {
        const favorites = [
            {
                "product_id": 2,
                "name": "boxing bag",
        ...
        cy.window().then((window) => {
            ...
            expect(storedFavorites).to.exist;
            expect(storedFavorites).to.be.an('array').that.is.not.empty;
